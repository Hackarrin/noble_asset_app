...
import 'package:nobleassets/utils/bookings/shortlet.dart';
...
class ShortletFilter extends StatefulWidget {
  const ShortletFilter({Key? key}) : super(key: key);

  @override
  _ShortletFilterState createState() => _ShortletFilterState();
}

class _ShortletFilterState extends State<ShortletFilter> {
            ...
            Marker marker = Marker(
                ...
                position: LatLng(
                    ...
                    num.tryParse(item["lon"].toString())?.toDouble() ??
                        _center.longitude),
                onTap: () {
                  Sheets.showItem(item, type: "shortlet");
                });
            markers.add(marker);
          }
          setState(() {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: GestureDetector(
          onTap: () async {
            final res = await ShortletModals.filters(
                      ...
                      children: [
                        ...
                        FittedBox(
                          child: Widgets.buildText(
                              "${Helpers.formatDistanceDate(filter["startDate"]!, filter["endDate"]!)} (${filter["duration"]} day${(num.tryParse(filter["duration"].toString())?.toInt() ?? 0) > 1 ? "s" : ""}). ${filter["adults"]} adult${(num.tryParse(filter["adults"].toString())?.toInt() ?? 0) > 1 ? "s" : ""}",
                              context,
        ...
        actions: [
          GestureDetector(
            onTap: () async {
              final res = await ShortletModals.filterOther(
                      ...
                      child: ListView.builder(
                        itemBuilder: (BuildContext context, int index) {
                          return Padding(
                            ...
                            child: ShortletItem(item: _data[index]),
                          );
                        },
